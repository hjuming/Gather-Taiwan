-- Wave 03-A: a private roster may be visible through its bare slug, but only
-- the holder of an invitee-specific capability can change that invitee's
-- response. Plaintext capabilities are returned once and only their hash is
-- retained in event_invitation_targets.

-- A legacy shared-link visitor could claim a target by writing a browser key
-- through the old name fallback. Those keys are not personal capabilities, so
-- invalidate them before enabling the token-only response contract.
with invalidated as (
  update public.event_invitation_targets target
  set guest_key_hash = null,
      updated_at = statement_timestamp()
  from public.events event
  where event.id = target.event_id
    and event.status = 'published'
    and event.visibility = 'private'
    and event.invite_only
    and target.revoked_at is null
    and target.guest_key_hash is not null
  returning target.id, target.event_id
)
insert into public.audit_logs (event_id, action, after_state)
select
  event_id,
  'event_invitation.legacy_token_invalidated',
  jsonb_build_object('target_id', id)
from invalidated;

create or replace function public.organizer_issue_event_invitation_token(
  p_target_id uuid
)
returns text
language plpgsql
...
begin
  ...
  if not exists (
    ...
    where e.id = v_target.event_id
      and e.status = 'published'
      and e.visibility = 'private'
      and e.invite_only
  ) then
    raise exception 'personal invitation links require a published private invite-only event'
      using errcode = '42501';
  end if;

  v_token := encode(extensions.gen_random_bytes(24), 'hex');
  update public.event_invitation_targets
  set guest_key_hash = public.hash_invitee_key(v_token),
      updated_at = statement_timestamp()
  where id = v_target.id;

  insert into public.audit_logs (actor_user_id, event_id, action, after_state)
  values (
    v_actor_user_id, v_target.event_id, 'event_invitation.token_issued',
    jsonb_build_object('target_id', v_target.id)
  );
...
-- Existing shared-link callers used four arguments, including a mutable name.
-- Dropping that signature prevents it from being selected by PostgREST after
-- this migration; the replacement never inserts, renames, or re-keys targets.
drop function public.respond_to_event_invitation(text, text, text, text);
...
revoke all on function public.organizer_add_event_invitation_target(uuid, text) from public, anon, authenticated;
revoke all on function public.organizer_remove_event_invitation_target(uuid) from public, anon, authenticated;
revoke all on function public.organizer_issue_event_invitation_token(uuid) from public, anon, authenticated;
...
grant execute on function public.organizer_issue_event_invitation_token(uuid) to authenticated;
