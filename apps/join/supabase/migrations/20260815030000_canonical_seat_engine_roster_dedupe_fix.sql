-- Correct the private roster projection after canonical seat accounting A:
-- attending-only rules govern capacity, while any active invitation target
-- still owns the display row for a matching manual registration. This keeps
-- the private roster de-duplicated without subtracting pending invitees from
-- occupied capacity.
...
begin
  ...
  -- Capacity accounting remains attending-only.
  ...
  -- regardless of response, so a pending invite and manual row render once.
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        'id', roster.roster_id,
