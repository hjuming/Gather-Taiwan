...
- Current Wave 0：`CLOSED（evidence-boundary closure）`；Wave 1：`ACCEPTED／CLOSED（Release baseline）`；Wave 2：`BLOCKED／未啟動`。
...
- 下一個開發波次是 Wave 2 organizer roster closure；先做 read-only code discovery，再由 owner 凍結 source／test／docs exact allowlist。既有 RPC／RLS 基礎不代表 app API／UI／測試已完成。
