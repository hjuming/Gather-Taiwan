...
| 報名 App | `https://gather.wedopr.com/app/*`（Workers Route，2026-08-06 起改為同網域路徑，前為獨立子網域） | 現行 Worker version `a5123237-c470-4047-a953-353aad4dc6a9`；正式 `/app/auth` 已載入 `index-BeJCmF3D.js`，私密活動頁已 read-back 完整 OG metadata 與 `noindex` |
...
- Email magic-link profile bootstrap（2026-08-15）：`935de99` 讓 magic-link session 自動補建
  `public.users` profile，並保存送出 email 時的顯示名稱；目前 OTP request 以
  `redirect_to=https://gather.wedopr.com/app/auth?redirect=…` 保留原活動導向，
  確認成功後由登入頁自動導回。歷史部署曾使用 `/app/`；若使用者仍收到舊的 `localhost:3000` 連結，
  先確認信件是否為部署前寄出。本輪 B 身分 `gather@wedopr.com` 已成功建立 session，
...
- Email／密碼與登入 email（2026-08-15）：`29e7567`、`4fd5c9f`、`a8cb0b2`、`958f20d`、
  `863f226`、`7b3cc41`、`ce19ef6` 已推送並部署。登入頁明確區分
  ...
  `double_confirm_changes` 設定仍未查證。2026-08-15 已套用
  `20260815050000_synthetic_line_verified_guard`：歷史 synthetic 身份的
  `public.users.email_verified_at` 已清除，保留 managed `auth.users`、email、LINE linkage 與
  其他關聯資料；`sync_verified_email()` 對 synthetic LINE email fail-closed，僅 authenticated
  可執行。
