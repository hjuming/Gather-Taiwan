#!/usr/bin/env bash
# Runs the P1-05 RBAC behavior verification against the Gather cloud project.
# See verify-p1-04.sh for the connection convention.
set -euo pipefail
cd "$(dirname "$0")/.."
...
PGCONNECT_TIMEOUT=15 PGPASSWORD="$SUPABASE_DB_PASSWORD" psql "$DB_URL" -f scripts/verify-p1-05-rls.sql
