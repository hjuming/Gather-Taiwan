...
try {
  ...
  const tokens = await asOrganizer(async (tx) => {
    ...
    return { primaryTargetId, oldPrimaryToken, primaryToken, duplicateManualToken, secondToken, thirdToken, otherEventToken };
  });
  ...
  await assert(duplicateManualAttending.payload.attending_count === 6, "matching invitation response should restore exactly one manual attendee seat");
  ...
  await assert(attending.payload.attending_count === 7, "new token should update only its own invitee");
  ...
  await asRole("anon", (tx) => tx`select public.respond_to_event_invitation(${slug}, ${tokens.secondToken}, 'attending')`);
  await expectSqlState(
    () => asRole("anon", (tx) => tx`select public.respond_to_event_invitation(${slug}, ${tokens.thirdToken}, 'attending')`),
    "53300",
  ...
  await assert(after.payload.attending_count === 8, "aggregate count must match de-duplicated roster at capacity");
  await assert(after.payload.invitees?.filter((invitee) => invitee.display_name === "日月MING").length === 1, "manual/invitation duplicate must remain collapsed after response");
