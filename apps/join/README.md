...
- Wave 2 已完成 local scope：`src/lib/api.ts` wrappers、`src/components/RosterManager.tsx` online-only actions、forward-only migration、focused tests、12-case local verifier 與 residue=0 read-back；remote／production、Cloudflare、merge／deploy 仍不在本輪 scope。
- Wave 2 可重跑入口與 exact local boundary 見 `scripts/verify-organizer-registration-wave2.mjs`、`scripts/cleanup-organizer-registration-wave2-fixtures.mjs` 與 `implementation-control-log.md` EOF current section。
